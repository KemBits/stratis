...
###   This script attempts to download the signature file SHA256SUMS.asc from stratis.org
...
WORKINGDIR="/tmp/stratis"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://stratis.org/bin/"
VERSIONPREFIX="stratis-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'stratis-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Stratis Core binary release signing key installed?"
   fi
