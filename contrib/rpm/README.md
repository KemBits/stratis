...
The RPM spec file provided here is for Stratis-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://stratis.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Stratis git repository](https://github.com/stratis/stratis) but are not part of
...
Currently only `Source10` is used, for the example `stratis.conf` file.
...
in the stratis git repository. At present only an SVG version of the Stratis
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the stratis
...
needed when 0.12.1 is released, a proper fix is already in the Stratis git
master branch.
...
idea to build Stratis Core against a static version of that library compiled
...
The `stratis.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `stratis-server` package. At this point, most RPM based
...
The `stratis-server` package should have SELinux support. How to properly do
...
Stratis-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/stratis/). He has been
packaging Stratis for Fedora at least since 2012.
...
4. I renamed the `stratis` package that contains the Qt GUI to `stratis-core` as
that appears to be how the general population refers to it, in contrast to
`stratis-xt` or `stratis-classic`. I wanted to make sure the general population
