...
*Setup instructions for a Gitian build of Stratis Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Stratis
...
- [Building Stratis Core](#building-stratis-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-stratis-host/README.md).
...
Clone the git repositories for stratis and Gitian.
...
in the stratis repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Stratis Core
----------------

To build Stratis Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the stratis repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stratis/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stratis/stratis
...
For example:
```bash
URL=https://github.com/laanwj/stratis.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit stratis=${COMMIT} --url stratis=${URL} ../stratis/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stratis=${COMMIT} --url stratis=${URL} ../stratis/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stratis=${COMMIT} --url stratis=${URL} ../stratis/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stratis git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stratis/contrib/gitian-descriptors/*|sort|uniq )
...
STRAPATH=/some/root/path/stratis
SIGPATH=/some/root/path/stratis-detached-sigs

./bin/gbuild --url stratis=${STRAPATH},signature=${SIGPATH} ../stratis/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stratis-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stratis-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stratis-osx-build.assert
```
...
[stratis-core/gitian.sigs](https://github.com/stratis-core/gitian.sigs/) repository, or if that's not possible create a pull
